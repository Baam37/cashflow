...
const useTransactions = () => {
  ...
  const reset = () => { localStorage.removeItem("cashflow_txs_de"); setTxs([]); };
  return { txs, add, remove, reset };
};
...
const BalkenDiagramm = ({ txs }: { txs: Transaction[] }) => {
  ...
  return (
    <div className="chart-card">
      ...
      {monate.length === 0 ? (
        <div className="empty-chart">Noch keine Daten</div>
      ) : (
...
const KreisDiagramm = ({ txs }: { txs: Transaction[] }) => {
  ...
  return (
    <div className="chart-card">
      ...
      {total === 0 ? (
        <div className="empty-chart">Noch keine Ausgaben</div>
      ) : (
...
const ResetConfirm = ({ onConfirm, onCancel }: { onConfirm: () => void; onCancel: () => void }) => (
  <div className="panel-overlay">
    <div className="reset-modal">
      <div className="reset-title">ALLE DATEN LÖSCHEN?</div>
      <div className="reset-sub">Diese Aktion kann nicht rückgängig gemacht werden.</div>
      <div className="reset-buttons">
        <button className="reset-cancel" onClick={onCancel}>ABBRECHEN</button>
        <button className="reset-confirm" onClick={onConfirm}>LÖSCHEN</button>
      </div>
    </div>
  </div>
);
...
export default function App() {
  const { txs, add, remove, reset } = useTransactions();
  ...
  return (
    <>
      ...
      <div className="app">
            ...
            <div className="screen">
              ...
              {txs.length === 0 ? (
                <div className="empty-state">
                  ...
                  <div className="empty-text">Noch keine Transaktionen</div>
                  <div className="empty-sub">Tippe auf + um zu beginnen</div>
                </div>
              ) : (
                <>
        ...
        {showReset && <ResetConfirm onConfirm={() => { reset(); setShowReset(false); }} onCancel={() => setShowReset(false)} />}
      </div>
    </>
  );
}
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .app-main { flex: 1; overflow-y: auto; padding: 20px; padding-bottom: 120px; }
  ...
  .empty-chart { font-family: var(--mono); font-size: 11px; color: var(--muted); text-align: center; padding: 24px 0; }
  ...
  .empty-text { font-family: var(--sans); font-size: 15px; color: var(--muted); }
  .empty-sub { font-family: var(--mono); font-size: 11px; color: var(--muted); opacity: 0.6; }
  ...
  .reset-btn { position: fixed; bottom: 36px; right: 20px; background: var(--surface2); border: 1px solid var(--border); color: var(--muted); font-family: var(--mono); font-size: 10px; letter-spacing: 0.1em; padding: 8px 12px; border-radius: 100px; cursor: pointer; transition: all 0.2s; }
  ...
  .reset-modal { background: var(--surface); border: 1px solid var(--border); border-radius: 20px; padding: 28px 24px; width: calc(100% - 40px); max-width: 400px; margin-bottom: 40px; text-align: center; animation: slide-up 0.3s cubic-bezier(0.4,0,0.2,1); }
  .reset-title { font-family: var(--mono); font-size: 13px; letter-spacing: 0.15em; color: var(--text); margin-bottom: 10px; }
  .reset-sub { font-family: var(--mono); font-size: 11px; color: var(--muted); margin-bottom: 24px; }
  .reset-buttons { display: flex; gap: 10px; }
  .reset-cancel { flex: 1; padding: 14px; background: var(--surface2); border: 1px solid var(--border); color: var(--muted); font-family: var(--mono); font-size: 11px; letter-spacing: 0.1em; border-radius: 12px; cursor: pointer; }
  .reset-confirm { flex: 1; padding: 14px; background: var(--red); border: none; color: #fff; font-family: var(--mono); font-size: 11px; letter-spacing: 0.1em; border-radius: 12px; cursor: pointer; }
